use trust_score_generators::exchange_application_tsg::{
    trivial_tsg
};
use utility::parse_messages;
...
fn main() -> Result<()> {
    ...
    let (tn_ver, wn_ver) = trivial_tsg::tsg_assume_group(msgs_and_pks, trivial_tsg::HonestWho::TransactingNodes).unwrap();

...
// 'reputation', [0,1], an unambiguous/simplistic measure of the honesty of their
...
pub struct Identity<C, I> {
    ...
    pub id_info: I,
    /// A Map from public keys to the perceived reputation of the associated participant
    pub reputation_map: ReputationMap,
    /// The minimum trust this participant needs to have to engage with another participant.
    /// For an organization, this is the minimum average reputation of the participants
    /// needed to allow the transaction.
    pub user_reputation_threshold: f32,
    /// The trust this participant has in a unknown participant
    pub user_default_reputation: f32
}

impl<C,I> Identity<C,I> {
    pub fn update_reputation(&mut self, new_estimates: Verdict){
    ...
    /// Average out the participants reputation scores and checks if this
    /// passes the reputation threshold
    pub fn check_avg_participants(&mut self, participants: &Vec<String>) -> bool{
        participants
            .iter()
            .for_each(|p| self.add_if_not_already(p)); 
        
        let reputation_sum: f32 = participants
            .iter()
            .map(|p| self.reputation_map.get(p).unwrap())
            .map(|r| self.calculate_score(r))
            .sum();
        return (reputation_sum / participants.len() as f32) >= self.user_reputation_threshold;
    }

    /// Checks if all participants pass the reputation threshold
    ...
    /// Checks if a participant passes the reputation threshold
    pub fn check_participant(&mut self, participant: &str) -> bool{
        self.add_if_not_already(participant); 

        let reputation = self.calculate_score(self.reputation_map.get(participant).unwrap());
        return reputation >= self.user_reputation_threshold;
    }
    ...
    // Because the default value in the reputation map is (0.0, 0), a div by zero
    // error must be avoided. Conveniently, (_, 0) means we should use the default value.
    pub fn calculate_score(&self, components: &ReputationComponents) -> f32 {
        if components.1 == 0{
            return self.user_default_reputation;
        }
        return components.0 / components.1 as f32;
    }

    pub fn get_reputation_scores_string(&self) -> String {
    ...
    where T : TsgFramework {
        ...
        self.update_reputation(tn_verdicts.clone());
        self.update_reputation(wn_verdicts.clone());
    }
}

// Reputation is calculated from reputation estimates. One reputation
// estimate of 0.5 causes the ReputationScore = (0.5,1), and the score
// is 0.5/1=0.5. After another estimate of 1.0, the ReputationScore = (1.5,2),
// and the score is 1.5/2=0.75
pub type ReputationComponents = (f32, usize);

/// Maps public keys to a reputation score
pub type ReputationMap = HashMap<String, ReputationComponents>;
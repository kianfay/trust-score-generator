...
fn test_trivial_tsg_assume_participants_all_true() {
    ...
    let tx = template_tx(vec!["[true, true]"; 2]);
    let parsed_msgs_and_pks = parse_messages::parse_messages(&tx).unwrap();
    let (tn_ver, wn_ver) = trivial_tsg::tsg_assume_group(parsed_msgs_and_pks, trivial_tsg::HonestWho::TransactingNodes).unwrap();
...
fn test_trivial_tsg_assume_participants_witnesses_honest() {
    ...
    let tx = template_tx(vec!["[true, false]"; 2]);
    let parsed_msgs_and_pks = parse_messages::parse_messages(&tx).unwrap();
    let (tn_ver, wn_ver) = trivial_tsg::tsg_assume_group(parsed_msgs_and_pks, trivial_tsg::HonestWho::Witnesses).unwrap();
...
fn test_trivial_tsg_know_outcome_all_true() {
    ...
    let (tn_ver, wn_ver) = trivial_tsg::tsg_know_outcome(parsed_msgs_and_pks, known_outcome).unwrap();
